    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(address,index) in addressInfo" :key="address.id">
        ...
        <p @click="changeDefault(address,addressInfo)">
          ...
          <span class="s3" v- v-show="address.isDefault==1">默认地址 </span>
        </p>
      </div>
      <div class="line"></div>
      ...
      <div class="detail" >
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="(order,index) in userOrder.detailArrayList" :key="index">
          <li>
            <img :src=order.imgUrl alt="" class="chicun">
          </li>
          <li>
            <p>
              {{order.skuName}}</p>
      ...
      <div class="bbs">
        <h5>买家留言：</h5>
        <textarea placeholder="建议留言前先与商家沟通确认" class="remarks-cont" v-model="msg">{{msg}}</textarea>
...
<script>
  ...
  export default {
    ...
    mounted() {
      this.$store.dispatch('getAderss');
      this.$store.dispatch('getOrder')
    },
    computed:{
      ...mapState({
        addressInfo:state=>state.trade.trade,
