...
export default {
  ...
  //mounted() 生命周期：将上面的template挂载到整个dom中后
  //组件挂载完毕:可以向服务器发请求
  mounted() {
  ...
  methods: {
    ...
    goSearch(event) {
      //最好的解决方案:编程式导航+事件委派
      //存在一些问题:事件委派，是把全部的子节点【h3、dt、dl、em】的事件委派给父亲节点,父亲节点有@click
      ...
      // console.log(event.target)// <a data-v-18b3c0cc data-categoryname="手机" data-category1id="2">手机</a>

      let element = event.target
      //获取到当前出发这个事件的节点【h3、a、dt、dl】，需要带有data-categoryname这样节点【一定是a标签】
      //节点有一个属性dataset属性，可以获取节点的自定义属性与属性值
      // console.log(element.dataset) //DOMStringMap {v-18b3c0cc: '', categoryname: '手机', category1id: '2'}
      ...
      if (categoryname) {
        ...
        //判断:如果路由跳转的时候,带有params参数,~去
        if (this.$route.params) {
          location.params = this.$route.params
          //动态给location配置对象添加query属性
          //整理完参数
          location.query = query
          // console.log(location.query)//{categoryName: '手机', category1Id: '2'}
          //路由跳转
          // console.log(location) //{name: 'search', query: {{category1Id: "2" categoryName: "手机"}} }
