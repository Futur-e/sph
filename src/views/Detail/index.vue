  ...
  <div class="detail">
    ...
    <section class="con">
      ...
      <div class="conPoin">
        ...
        <span v-show="skuInfo.skuAttrValueList[0].valueName">{{skuInfo.skuAttrValueList[0].valueName}}</span>
      </div>
      <!-- 主要内容区域 -->
      <div class="mainCon">
        <!-- 左侧放大镜区域 -->
            ...
            <div class="chooseArea">
              ...
              <dl v-for="(attrList,index) in spuSaleAttrList" :key="attrList.id">
                ...
                <dd changepirce="0"
                    ...
                    v-for="(attr,index) in attrList.spuSaleAttrValueList"
                    :key="attr.id"
...
<script>
  ...
  import {reqShopCarData} from "@/api";
  ...
  export default {
    ...
    mounted() {
      this.$store.dispatch('getSkuIdList',this.$route.params.skuId)
    },
    computed:{
      ...mapGetters(['categoryView','skuInfo','spuSaleAttrList'])
    },
    methods:{
      changeActive(attrValue,attrList){
      ...
      async getIdAndNum(){
        try {
          let result = await this.$store.dispatch('addIdAndNum',{skuId:this.$route.params.skuId,skuNum:this.skuNumber});
              sessionStorage.setItem('GETNUM',JSON.stringify(this.skuInfo))
